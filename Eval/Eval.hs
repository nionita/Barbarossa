...
kingPlace ep p = [ kcd ]
    where !kcd = (mpl - ypl) `unsafeShiftR` epMaterBonusScale ep
          ...
          (!mpl, !ypl)
              | moving p == White = (kingMaterBonusWhite mpawns mks, kingMaterBonusBlack ypawns yks)
              | otherwise         = (kingMaterBonusBlack mpawns mks, kingMaterBonusWhite ypawns yks)
          !mpawns  = pawns p .&. me p
          !ypawns  = pawns p .&. yo p
...
kingMaterBonusWhite :: BBoard -> Square -> Int
kingMaterBonusWhite !myp !ksq = prx
    where !prx = prxWA + prxWH
...
kingMaterBonusBlack :: BBoard -> Square -> Int
kingMaterBonusBlack !myp !ksq = prx
    where !prx = prxBA + prxBH
