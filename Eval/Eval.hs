...
mobDiff p _ mide = mide <+> MidEnd md ed
    where !myPawns = me p .&. pawns p
          !yoPawns = yo p .&. pawns p
          (!myblo, !yoblo)
          --    | moving p == White = (blockedW myPawns (occup p), blockedB yoPawns (occup p))
          --    | otherwise         = (blockedB myPawns (occup p), blockedW yoPawns (occup p))
              | moving p == White = (blockedW myPawns (pawns p), blockedB yoPawns (pawns p))
              | otherwise         = (blockedB myPawns (pawns p), blockedW yoPawns (pawns p))
          !myok = complement (myblo .|. yoPAttacs p .|. (kings p .&. me p))
          !yook = complement (yoblo .|. myPAttacs p .|. (kings p .&. yo p))
          blockedW mep yop = mep .&. (yop `unsafeShiftR` 1)
          blockedB mep yop = mep .&. (yop `unsafeShiftL` 1)
