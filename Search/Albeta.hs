...
pvZeroW !_ !b !d _ | d <= 0 = do
    ...
    return $! pathFromScore ("pvQSearch 2:" ++ show v) v
...
pvInnerLoop :: Int 	-- current beta
            ...
            -> Move	-- move to search
            -> Search NodeState
pvInnerLoop b d prune nst e = timeToAbort nst $ do
...
pvInnerLoopZ :: Int 	-- current beta
            ...
            -> Bool	-- reduce in LMR?
            -> Search NodeState
pvInnerLoopZ b d prune nst e redu = timeToAbort nst $ do
         -- What about TT & killer moves???
         let !canPrune = canPruneMove (cpos nst) e
         if prune && canPrune
            then return $! nst { movno = movno nst + 1 }
