...
options = [
        ...
        Option "b" ["player1"] (ReqArg setPlayer2 "STRING") "Configuration file for player 2",
        ...
        Option "d" ["depth"]   (ReqArg addDepth "STRING")  "Oracle search depth",
...
theOptions = do
    ...
    where header = "Usage: " ++ idName
              ++ " [-c CONF] [-m [-p PARAMS] -q PARAMS] [-t THREADS] [-i FENFILE [-s SKIP][-f FENS]] [-o OUTFILE] [-d DEPTH]"
          idName = "SelfPlay"
...
matchFile opts dir = do
    ...
    case mids of
        Nothing -> do
            liftIO $ putStrLn "For a match we need 2 config s as playes"
            return (0, 0, 0)
        Just (id1, id2) -> do
            ctxLog LogInfo $ "Players from directory " ++ dir
            ctxLog LogInfo $ "Players 1 " ++ id1
            ctxLog LogInfo $ "Players 2 " ++ id2
            (hi, ho) <- liftIO $ do
    ...
    where go i (id1, chg1) (id2, chg2) = do
              ...
              if null path
                 then do
                     ...
                     return $ GameAborted "Empty path when playing"	-- should not happen when i > 0
                 else case msc of
                          Nothing -> do
                              ...
                              if sc == mateScore && length path == 1	-- mate in 1
                                 then if tacticalPos p
                                         ...
                                         then do
                                             ...
                                             -- Here we should check more termination reasons
                                             -- like patt, 3 repetitions and no mating material
                                             hi <- liftIO newHist
