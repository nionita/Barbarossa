...
instance CollectParams EvalParams where
    ...
    npColInit = EvalParams {
                    epMovingMid  = 162,		-- after Clop optimisation
                    epMovingEnd  = 132,		-- with 3700 games at 15+0.25 s
                    epMaterMinor = 1,
                    epMaterRook  = 3,
                    epMaterQueen = 15,
                    epMaterScale = 1,
...
instance CollectParams EvalWeights where
    ...
    npColInit = EvalWeights {
          ...
          ewCenterKAtts     = tme  0 60,
          ewAdvAtts         = tme  3 16,
          ewIsolPawns       = tme (-50) (-150),
          ewIsolPassed      = tme (-60) (-150),
